...
public class HologramManager {
    ...
    /** npcId -> was LookClose present before freezing, so unfreeze only restores it if it was actually removed by us. */
     ...
     *
     * Citizens' LookClose trait is removed (not just disabled) while frozen,
     * since its enable/disable method signature has changed across Citizens
     * versions; removing/re-adding the trait via the stable addTrait/
     * removeTrait API works consistently across versions. If the NPC didn't
     * have LookClose to begin with, unfreezing does not add it.
     *
     * @param npc the NPC to freeze/unfreeze
     * @param facing if freezing, the direction (yaw/pitch) to lock the NPC
     *               to - normally the admin's own look direction at the
    ...
    public boolean toggleFreeze(NPC npc, org.bukkit.util.Vector facing) {
        ...
        if (frozenNpcIds.contains(id)) {
            ...
            if (hadLookCloseBeforeFreeze.remove(id)) {
                npc.addTrait(net.citizensnpcs.trait.LookClose.class);
            }
            return false;
        } else {
            frozenNpcIds.add(id);
            if (npc.hasTrait(net.citizensnpcs.trait.LookClose.class)) {
                ...
                npc.removeTrait(net.citizensnpcs.trait.LookClose.class);
            }
            // Lock the NPC facing the given direction (the admin's look
            // direction at the moment the command was run). Since this
            // rotates the actual NPC entity server-side, every player sees
            // the same locked orientation - not a per-player client effect.
            if (npc.isSpawned() && facing != null) {
                org.bukkit.Location loc = npc.getEntity().getLocation();
                org.bukkit.util.Vector flat = facing.clone();
     ...
     * entity packets for this NPC. The Minecraft client's own entity
     * render distance / fog distance (controlled by the player's video
     * settings and the server's view-distance) is a separate, client-side
     * limit that a plugin cannot override; 128 blocks is the practical
     * ceiling this method can guarantee server-side.
     */
